...
import 'package:india_running/features/home/data/repositories/event_repository_impl.dart'; // Import concrete implementation
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  late final FetchEventsUseCase fetchEventsUseCase;
  late final EventRepository eventRepository;

  HomeBloc() : super(HomeInitial()) {
    // Manually instantiate dependencies
    eventRepository = EventRepositoryImpl(); // Use concrete implementation
    fetchEventsUseCase = FetchEventsUseCase(eventRepository);
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc(this.fetchEventsUseCase) : super(HomeInitial()) {
    on<FetchHomeEvents>((event, emit) async {
      emit(HomeLoading());
      try {
        final List<Event> events = await fetchEventsUseCase.call();
        emit(HomeLoaded(events));
      } catch (e) {
        emit(HomeError('Failed to fetch events.'));
      }
